...
***LED board lasers intersection genuary arduino.mp4
...
***old housing front.jpg

However, it was ugly! Since I felt the wiring and assembly was sketchy, I didn't feel safe keeping this sketchy thing powered on my nightstand. As soon as I had re-adjust something inside, and realized it would take an hour to do so, I lost my patience with this design. I decided to do a redesign.
...
Eventually I started a new model in OnShape. Instead of having two legs lift the LED panel housing up off the ground, it would look like this:
...
But it worked. I inserted the heat set inserts in with a soldering iron and screwed everything together. This was so much better than the previous design, it took only a few minutes to assemble, it felt like a consumer product (aside from all these wires hanging around)
...
This board's program isn't limited to doing just one thing. There are multiple sub-programs -- I call them "applets" -- that you can easily switch between.
...
Development-wise, this was an Arduino program, but made using [PlatformIO](https://platformio.org/) in Visual Studio Code instead of Arduino IDE. Just like how I discovered that ESP32 was 'like Arduino but way better', I found that PlatformIO was also 'like Arduino but way better'. It lets me make Arduino (or native) programs from the comforts of VSCode. Go check it out if you haven't heard of it!

I'm decently okay with how the applet code is organized: An applet is just a cpp and h file with a setup and loop function defined. It can include "system.h" to consume inputs like knobs rotations and presses, and draw to the screen using the LED "matrix" class instance. Then, it's #included and added to a list in main.cpp and given a name:
...
Some notes:
  - I developed a good deal of this on the PC using a 'simulator' I made using Raylib.
...
The design is based off the Egg TV from the "Weather Globe" boss that appears in Sonic Mania.

***weathersoncimania.mp4
